...
import { Info, Loader2, Map as MapIcon, MenuSquare, Zap, MapPin, TrendingDown, ArrowLeft } from 'lucide-react';
...
async function fetchNeighborhoods(baseUrl: string): Promise<NeighborhoodMapItem[]> {
    try {
        const res = await fetch(`${baseUrl}/api/map/neighborhoods?limit=200`, { cache: 'no-store' });
        if (!res.ok) return [];
...
export default async function MapaBairrosPage(props: { searchParams: Promise<{ m?: string; f?: string }> }) {
    const searchParams = await props.searchParams;
    const listMode = searchParams.m === 'lista';
    const critOnly = searchParams.f === 'criticos';
    const baseUrl = process.env.NEXT_PUBLIC_VERCEL_URL ? `https://${process.env.NEXT_PUBLIC_VERCEL_URL}` : 'http://localhost:3000';

    let neighborhoods = await fetchNeighborhoods(baseUrl);
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-gray-200 dark:border-gray-800 pb-6">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">
                            Círculos coloridos por nível de atraso médio (Prometido vs Real, 30 dias).
                        </p>
                    </div>

                    <div className="flex gap-2">
                        <div className="flex bg-gray-200 dark:bg-gray-800 p-1 rounded-lg">
                            <a href="/mapa/bairros" className={`px-4 py-2 rounded-md font-medium text-sm flex items-center gap-2 transition ${!listMode ? 'bg-white dark:bg-black shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
                                <MapIcon size={16} /> Mapa
                            </a>
                            <a href="/mapa/bairros?m=lista" className={`px-4 py-2 rounded-md font-medium text-sm flex items-center gap-2 transition ${listMode ? 'bg-white dark:bg-black shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
                                <MenuSquare size={16} /> Lista
                            </a>
                            <a href={critOnly ? '/mapa/bairros' : '/mapa/bairros?f=criticos'} className={`px-4 py-2 rounded-md font-medium text-sm flex items-center gap-2 transition ${critOnly ? 'bg-white dark:bg-black shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}>
                    ...
                    {listMode ? (
                        ...
                        <div className="w-full h-full rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm relative z-0">
                            <Suspense fallback={
                                ...
                                <NeighborhoodMapWrapper neighborhoods={neighborhoods} />
                            </Suspense>
                        </div>
                    )}
                </div>
